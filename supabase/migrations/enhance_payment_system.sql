-- Enhanced NicePay 결제시스템 스키마 업데이트
-- 생성일: 2024-01-15
-- 설명: 기존 payment_orders 테이블을 확장하고 새로운 테이블들을 추가

-- 1. 기존 payment_orders 테이블 확장
-- 새로운 컬럼 추가
ALTER TABLE payment_orders 
ADD COLUMN IF NOT EXISTS product_type VARCHAR(20) CHECK (product_type IN ('points', 'subscription')),
ADD COLUMN IF NOT EXISTS product_id VARCHAR(50),
ADD COLUMN IF NOT EXISTS currency VARCHAR(3) DEFAULT 'KRW',
ADD COLUMN IF NOT EXISTS pay_method VARCHAR(20) CHECK (pay_method IN ('CARD', 'BANK', 'VBANK', 'MOBILE')),
...
ALTER TABLE payment_orders ADD CONSTRAINT payment_orders_status_check 
    CHECK (status IN ('pending', 'approved', 'failed', 'cancelled', 'refunded', 'completed'));

-- 기존 payment_type 체크 제약조건 업데이트  
ALTER TABLE payment_orders DROP CONSTRAINT IF EXISTS payment_orders_payment_type_check;
ALTER TABLE payment_orders ADD CONSTRAINT payment_orders_payment_type_check 
    CHECK (payment_type IN ('monthly', 'addon', 'subscription', 'points'));

-- 2. 결제 거래 테이블 (payment_transactions) 생성
...
-- 4. 결제 설정 테이블 (payment_settings) 생성
...
-- payment_logs 인덱스
CREATE INDEX IF NOT EXISTS idx_payment_logs_payment_order_id ON payment_logs(payment_order_id);
...
SELECT 
    ...
    COUNT(CASE WHEN status IN ('approved', 'completed') THEN 1 END) as successful_orders,
    COUNT(CASE WHEN status = 'failed' THEN 1 END) as failed_orders,
    SUM(CASE WHEN status IN ('approved', 'completed') THEN amount_krw ELSE 0 END) as total_revenue,
    AVG(CASE WHEN status IN ('approved', 'completed') THEN amount_krw END) as avg_order_value
FROM payment_orders
...
SELECT 
    ...
    COUNT(*) as total_orders,
    COUNT(CASE WHEN status IN ('approved', 'completed') THEN 1 END) as successful_orders,
    SUM(CASE WHEN status IN ('approved', 'completed') THEN amount_krw ELSE 0 END) as total_spent,
...
SELECT 
    ...
    SUM(CASE WHEN status = 'completed' THEN refund_amount ELSE 0 END) as total_refund_amount,
    AVG(CASE WHEN status = 'completed' THEN refund_amount END) as avg_refund_amount
FROM refunds
GROUP BY DATE_TRUNC('day', created_at)
ORDER BY date DESC;

-- 테이블 코멘트 추가
COMMENT ON TABLE payment_orders IS 'NicePay 결제 주문 정보를 저장하는 테이블 (Enhanced)';
