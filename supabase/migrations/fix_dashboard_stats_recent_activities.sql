-- get_dashboard_stats 함수 수정 - recent_activities 및 technology_fields 수정 (updated)
  ...
  SELECT json_build_object(
    ...
    'efficiency_metrics', json_build_object(
      'login_to_report_rate', CASE 
        ...
        THEN ROUND(
          (SELECT COUNT(*) FROM ai_analysis_reports WHERE user_id = p_user_id AND created_at >= start_date)::float / 
          (SELECT COUNT(*) FROM user_activities WHERE user_id = p_user_id AND activity_type = 'login' AND created_at >= start_date)::float * 100, 1
        )
        ELSE 0 
      END,
      'search_to_report_rate', CASE 
        ...
        THEN ROUND(
          (SELECT COUNT(*) FROM ai_analysis_reports WHERE user_id = p_user_id AND created_at >= start_date)::float / 
          (SELECT COUNT(*) FROM search_history WHERE user_id = p_user_id AND created_at >= start_date)::float * 100, 1
        )
        ELSE 0 
      END,
      'total_logins', (
    ...
    'technology_fields', (
      ...
      FROM (
        SELECT 
          ...
          ROUND(COUNT(*)::float / (SELECT COUNT(*) FROM search_history WHERE user_id = p_user_id AND created_at >= start_date)::float * 100, 1) AS percentage
        FROM search_history 
        WHERE user_id = p_user_id 
          AND created_at >= start_date
        GROUP BY 
    ...
    'report_fields_top10', (
      ...
      FROM (
        SELECT 
          ...
          ROUND(COUNT(*)::float / (SELECT COUNT(*) FROM ai_analysis_reports WHERE user_id = p_user_id AND created_at >= start_date)::float * 100, 1) AS percentage
        FROM ai_analysis_reports 
        WHERE user_id = p_user_id 
          AND created_at >= start_date
        GROUP BY 
