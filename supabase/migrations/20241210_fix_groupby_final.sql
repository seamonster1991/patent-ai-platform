-- Fix GROUP BY error in dashboard stats function - Updated
-- This migration fixes the GROUP BY clause issues in recent activities
...
BEGIN
  ...
  -- Recent activities - Fixed to avoid GROUP BY issues
  recent_reports AS (
    ...
    FROM (
      SELECT r.id, r.report_name, r.invention_title, r.created_at, 
             r.application_number, r.analysis_type
      FROM ai_analysis_reports r
      WHERE r.user_id = p_user_id 
        AND r.created_at >= NOW() - period_interval
      ORDER BY r.created_at DESC 
  ...
  recent_searches AS (
    ...
    FROM (
      SELECT sh.id, sh.keyword, sh.created_at, sh.results_count
      FROM search_history sh
      WHERE sh.user_id = p_user_id 
        AND sh.created_at >= NOW() - period_interval
      ORDER BY sh.created_at DESC 
  ...
  SELECT json_build_object(
    ...
    'recent_activities', (
      SELECT json_build_object(
        'reports', rr.activities,
        'searches', rs.activities
      )
