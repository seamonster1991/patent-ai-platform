-- 기존 사용자 데이터를 업데이트하고 관련 테스트 데이터 생성
DO $$
DECLARE
    real_user_id UUID;
...
BEGIN
    ...
    IF real_user_id IS NOT NULL THEN
        ...
        -- public.users 테이블에 사용자 정보 추가 또는 업데이트
        ...
        VALUES (
            real_user_id,
            COALESCE(auth_user_email, 'test@example.com'),
            'Test User',
            'premium',
            150,
            25,
        ...
        ON CONFLICT (id) DO UPDATE SET
            ...
            total_detail_views = EXCLUDED.total_detail_views,
