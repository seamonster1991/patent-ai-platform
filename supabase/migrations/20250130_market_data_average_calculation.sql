-- Improve market data calculation to use average per user
-- Description: Change market data from simple aggregation to total activity / user count
...
BEGIN
  ...
  -- Get total active users count for market data calculation
  SELECT COUNT(DISTINCT user_id) INTO total_users_count
  FROM (
    SELECT user_id FROM patent_search_analytics 
    WHERE created_at >= NOW() - period_interval
    UNION
    SELECT user_id FROM ai_analysis_reports 
    WHERE created_at >= NOW() - period_interval
  ) active_users;

  -- Ensure we have at least 1 user to avoid division by zero
  ...
  -- Market data: total activity divided by user count for average per user
  market_search_counts AS (
    SELECT 
      created_at::date AS d, 
      ROUND(COUNT(*)::numeric / total_users_count, 2) AS cnt
    FROM patent_search_analytics
    WHERE created_at >= NOW() - period_interval
    GROUP BY created_at::date
  ),
  ...
  -- Market search fields: average per user
  market_search_field_counts AS (
    SELECT 
      COALESCE(technology_field, 'General') AS field,
      COALESCE(ipc_main_class, 'G') AS ipc_code,
      ROUND(SUM(search_count)::numeric / total_users_count, 2) AS search_count
    FROM search_keyword_analytics
  ...
  -- Market report fields: average per user
  ...
  SELECT json_build_object(
    ...
    -- Add metadata about market calculation
    'market_metadata', json_build_object(
      'total_active_users', total_users_count,
      'calculation_method', 'average_per_user',
      'period', p_period,
      'last_updated', NOW()
    )
  ) INTO result;
