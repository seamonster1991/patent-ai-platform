...
BEGIN
    ...
    -- JSON 템플릿 준비
    search_filter_json := '{"sort": "date", "order": "desc"}'::jsonb;
    
    -- 1. 검색 기록 생성 (최근 30일)
    FOR i IN 1..20 LOOP
        -- 동적 JSON 생성
        search_result_json := json_build_object(
            'patents', json_build_array(
                json_build_object(
                    'id', i::text,
                    'title', 'Test Patent ' || i
                )
    ...
    -- 로그인 활동
    FOR i IN 1..10 LOOP
        activity_data_json := json_build_object(
            'login_method', 'email',
            'success', true
        )::jsonb;
    ...
    FOR i IN 1..25 LOOP
        activity_data_json := json_build_object(
            'keyword', search_keywords[((i-1) % array_length(search_keywords, 1)) + 1],
    ...
    FOR i IN 1..15 LOOP
        activity_data_json := json_build_object(
            'report_name', report_titles[((i-1) % array_length(report_titles, 1)) + 1],
