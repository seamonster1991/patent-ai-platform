-- Add technology field columns to ai_analysis_reports table
...
BEGIN
    ...
    -- AI/Machine Learning
    IF combined_text ~* '(artificial intelligence|machine learning|neural network|deep learning|ai|ml|computer vision|natural language|nlp|chatbot|automation|algorithm|data mining|pattern recognition)' THEN
        field_result := 'AI/머신러닝';
        confidence_result := 0.9;
    -- IoT/Smart Devices
    ELSIF combined_text ~* '(internet of things|iot|smart device|sensor|wireless|bluetooth|wifi|connected device|smart home|wearable|embedded system)' THEN
        field_result := 'IoT/스마트기기';
        confidence_result := 0.85;
    -- Communication/Network
    ELSIF combined_text ~* '(5g|6g|communication|network|wireless|antenna|signal|transmission|protocol|telecommunication|mobile|cellular|radio frequency|rf)' THEN
        field_result := '통신/네트워크';
        confidence_result := 0.85;
    -- Semiconductor/Electronics
    ELSIF combined_text ~* '(semiconductor|chip|processor|microprocessor|integrated circuit|ic|transistor|memory|storage|electronic|circuit|silicon|wafer)' THEN
        field_result := '반도체/전자';
        confidence_result := 0.8;
    -- Biotechnology/Medical
    ELSIF combined_text ~* '(biotechnology|biotech|medical|pharmaceutical|drug|medicine|therapy|diagnosis|genetic|dna|protein|vaccine|clinical|healthcare)' THEN
        field_result := '바이오/의료';
        confidence_result := 0.8;
    -- Transportation/Automotive
    ELSIF combined_text ~* '(automotive|vehicle|car|truck|transportation|electric vehicle|ev|autonomous|self-driving|battery|engine|motor|navigation)' THEN
        field_result := '교통/자동차';
        confidence_result := 0.8;
    -- Blockchain/Fintech
    ELSIF combined_text ~* '(blockchain|cryptocurrency|bitcoin|fintech|financial technology|digital currency|smart contract|decentralized|crypto|payment)' THEN
        field_result := '블록체인/핀테크';
        confidence_result := 0.8;
    -- Energy/Environment
    ELSIF combined_text ~* '(renewable energy|solar|wind|battery|energy storage|green technology|environmental|sustainability|carbon|emission|clean energy)' THEN
        field_result := '에너지/환경';
        confidence_result := 0.75;
    -- Manufacturing/Industrial
    ELSIF combined_text ~* '(manufacturing|industrial|automation|robotics|3d printing|additive manufacturing|factory|production|assembly|quality control)' THEN
        field_result := '제조/산업';
        confidence_result := 0.75;
    -- Software/Application
    ELSIF combined_text ~* '(software|application|app|program|platform|system|interface|user experience|ux|ui|web|mobile|cloud|saas)' THEN
