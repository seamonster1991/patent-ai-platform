-- Fixed get_dashboard_stats function with correct column names
...
BEGIN
    ...
    -- Get market search technology field distribution
    SELECT jsonb_agg(
        jsonb_build_object(
            'field', technology_field,
            'ipc_code', ipc_code,
    ...
    -- Get recent searches (fixed column names)
    SELECT jsonb_agg(
        jsonb_build_object(
            'keyword', keyword,
    ...
    -- Get recent reports (fixed column names)
    SELECT jsonb_agg(
        jsonb_build_object(
