-- Admin Dashboard Database Schema (Final)
...
-- Drop existing admin tables if they exist to avoid conflicts
...
CREATE FUNCTION grant_signup_bonus(p_user_id UUID)
RETURNS TABLE(
    granted BOOLEAN,
...
CREATE FUNCTION get_point_grant_stats(p_user_id UUID)
RETURNS TABLE(
...
CREATE FUNCTION check_and_grant_monthly_free_points(p_user_id UUID)
RETURNS TABLE(
    granted BOOLEAN,
    points_amount INTEGER,
    message TEXT,
