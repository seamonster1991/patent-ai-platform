-- Fix patent field analysis functions - Version 2
-- 2025-01-31: Fix SQL aggregate and window function issues

-- Drop existing functions if they exist
DROP FUNCTION IF EXISTS get_top_patent_fields();
DROP FUNCTION IF EXISTS analyze_patent_fields_from_reports();

-- Recreate the analyze function with fixed SQL
    ...
    SELECT 
        jsonb_agg(
            jsonb_build_object(
                ...
                'category', category,
                'count', report_count,
                'percentage', ROUND((report_count * 100.0 / NULLIF(tc.total_reports, 0)), 2)
            )
            ORDER BY report_count DESC
        ),
        tc.total_reports
    INTO field_counts, total_analyzed
    FROM field_counts_raw fcr
