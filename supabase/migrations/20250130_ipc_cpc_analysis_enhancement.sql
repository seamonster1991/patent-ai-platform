-- IPC/CPC Analysis Enhancement for Dashboard
...
-- Update existing search_history records with technology field classification
UPDATE search_history 
SET (technology_field, field_confidence) = (
    SELECT field, confidence 
    FROM classify_technology_field(keyword)
)
WHERE technology_field IS NULL;
...
-- Create trigger function to automatically classify technology field for new searches
CREATE OR REPLACE FUNCTION trigger_classify_search_technology()
RETURNS TRIGGER AS $$
BEGIN
    -- Classify technology field if not provided
    IF NEW.technology_field IS NULL THEN
        SELECT field, confidence 
        INTO NEW.technology_field, NEW.field_confidence
        FROM classify_technology_field(NEW.keyword);
    END IF;
...
-- Create triggers for automatic classification
...
DROP TRIGGER IF EXISTS trigger_auto_classify_search ON search_history;
CREATE TRIGGER trigger_auto_classify_search
    BEFORE INSERT OR UPDATE ON search_history
    FOR EACH ROW
    EXECUTE FUNCTION trigger_classify_search_technology();
...
SELECT 
    ...
    technology_field,
    COUNT(*) as count,
    COUNT(*) * 100.0 / SUM(COUNT(*)) OVER(PARTITION BY 'search', 'individual') as percentage
FROM search_history 
...
SELECT 
    ...
    technology_field,
    COUNT(*) as count,
    COUNT(*) * 100.0 / SUM(COUNT(*)) OVER(PARTITION BY 'search', 'market') as percentage
FROM search_history 
WHERE technology_field IS NOT NULL 
    AND created_at >= NOW() - INTERVAL '100 days'
GROUP BY technology_field
...
SELECT 
    ...
    technology_field,
    COUNT(*) as count,
    COUNT(*) * 100.0 / SUM(COUNT(*)) OVER(PARTITION BY 'report', 'individual') as percentage
...
SELECT 
    ...
    technology_field,
    COUNT(*) as count,
    COUNT(*) * 100.0 / SUM(COUNT(*)) OVER(PARTITION BY 'report', 'market') as percentage
FROM ai_analysis_reports 
WHERE technology_field IS NOT NULL 
    AND created_at >= NOW() - INTERVAL '100 days'
GROUP BY technology_field;
