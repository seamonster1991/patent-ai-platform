-- Update get_dashboard_stats function to use correct table names and implement all requirements
...
BEGIN
    -- Parse period to interval (default 100 days for data retention policy)
    ...
    -- Get user search count from patent_search_analytics
    SELECT COUNT(*)
    ...
    -- Get user report count from ai_analysis_reports
    SELECT COUNT(*)
    ...
    -- Get user search technology field distribution from technology_fields array
    SELECT jsonb_agg(
        jsonb_build_object(
    ...
    FROM (
        SELECT 
            COALESCE(field, '기타') as technology_field,
            COUNT(*) as count,
            ROUND((COUNT(*) * 100.0 / SUM(COUNT(*)) OVER())::numeric, 1) as percentage
        FROM (
            SELECT UNNEST(technology_fields) as field
            FROM patent_search_analytics
            WHERE (p_user_id IS NULL OR user_id = p_user_id)
                AND created_at >= NOW() - period_interval
                AND technology_fields IS NOT NULL
                AND array_length(technology_fields, 1) > 0
        ) expanded_fields
        GROUP BY field
        ORDER BY count DESC
        LIMIT 10
    ...
    -- Get user report technology field distribution from technology_fields array
    SELECT jsonb_agg(
        jsonb_build_object(
            'field', technology_field,
            'count', count,
    ...
    -- Get recent search terms (top 10) from patent_search_analytics
    SELECT jsonb_agg(
        jsonb_build_object(
            'keyword', search_query,
            'created_at', created_at,
            'technology_fields', COALESCE(technology_fields, ARRAY[]::text[]),
            'results_count', COALESCE(results_count, 0),
            'search_type', COALESCE(search_type, 'general')
        )
    )
    ...
    -- Get recent report titles (top 10) from ai_analysis_reports
    SELECT jsonb_agg(
        jsonb_build_object(
            ...
            'analysis_type', COALESCE(analysis_type, 'market')
        )
    )
    INTO recent_reports
    FROM (
    ...
    -- Build final result with all required data
...
COMMENT ON FUNCTION get_dashboard_stats(uuid, text) IS 'Enhanced dashboard statistics with IPC/CPC technology field analysis, recent activities, and 100-day data retention support';