-- Fix efficiency metrics calculation in get_dashboard_stats function
-- This migration fixes the incorrect JOIN logic in user_stats CTE
...
BEGIN
  ...
  WITH daily_searches AS (
    SELECT 
      DATE(ua.created_at) AS search_date,
  ...
  market_daily_searches AS (
    SELECT 
      DATE(ska.last_searched_at) AS search_date,
      SUM(ska.search_count) AS search_count
    FROM search_keyword_analytics ska
    WHERE ska.last_searched_at >= NOW() - period_interval
    GROUP BY DATE(ska.last_searched_at)
    ORDER BY search_date
  ),
  ...
  search_field_counts AS (
    SELECT 
      COALESCE(ska.technology_field, 'General') AS field,
      COALESCE(ska.ipc_main_class, 'G') AS ipc_code,
      SUM(ska.search_count) AS search_count
    FROM search_keyword_analytics ska
    JOIN user_activities ua ON ua.search_query = ska.keyword
    WHERE ua.user_id = p_user_id
      AND ua.activity_type = 'search'
      AND ua.created_at >= NOW() - period_interval
    GROUP BY COALESCE(ska.technology_field, 'General'), COALESCE(ska.ipc_main_class, 'G')
  ),
  ...
  -- Fixed recent activities to include both reports and searches
  recent_reports AS (
    SELECT json_agg(
      json_build_object(
        'id', r.id,
