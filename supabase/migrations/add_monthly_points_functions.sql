...
BEGIN
  ...
  WHERE user_id = p_user_id
    ...
    AND description = 'Monthly free points'
  ...
  IF v_last_grant_date IS NULL THEN
    -- 포인트 트랜잭션 기록
    INSERT INTO point_transactions (user_id, transaction_type, amount, description, expires_at)
    VALUES (p_user_id, 'earned', v_points_amount, 'Monthly free points', v_expires_at);
...
BEGIN
  ...
  IF NOT v_already_granted THEN
    -- 포인트 트랜잭션 기록
    INSERT INTO point_transactions (user_id, transaction_type, amount, description, expires_at)
    VALUES (p_user_id, 'earned', v_points_amount, 'Signup bonus', v_expires_at);
...
BEGIN
  -- 신규 가입 보너스 확인
  SELECT 
    EXISTS(SELECT 1 FROM point_transactions WHERE user_id = p_user_id AND description = 'Signup bonus'),
    (SELECT created_at FROM point_transactions WHERE user_id = p_user_id AND description = 'Signup bonus' LIMIT 1)
  ...
  FROM point_transactions
  WHERE user_id = p_user_id AND description = 'Monthly free points';
